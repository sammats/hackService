 ...
 * @api public
 */
function AdskStrategy(options, verify) {
    if (!options.returnURL) {
        throw new Error('OpenID authentication requires a returnURL option');
    }

    if (!verify) {
        throw new Error('OpenID authentication strategy requires a verify callback');
    }
    ...
    if (options.profile) {
        var ax = new openid.AttributeExchange({
            ...
            // oxygen parameters
            'http://axschema.org/autodesk/eidmguid': 'required',
            'http://axschema.org/autodesk/userid': 'required',

            //Analytics
            'http://axschema.org/autodesk/pref/emailoptin': 'required'
        });
        extensions.push(ax);
    }
    ...
    if (options.register) {
        var register = new adskopenid.Register();
        extensions.push(register);
    }

    if (options.customui) {
        var customui = new adskopenid.CustomUI();

        if (options.register) {
            customui.requestParams['openid.customui.x_ui_type'] = 'consumer';
        }

        extensions.push(customui);

        var authtype = new adskopenid.AuthType();
        extensions.push(authtype);
    }
 ...
 */
util.inherits(AdskStrategy, passport.Strategy);
 ...
 * @api protected
 */
AdskStrategy.prototype.authenticate = function(req) {
    ...
    if ((req.query && req.query['openid.mode']) || (req.body && req.body['openid.mode'])) {
        ...
        if (req.query['openid.mode'] === 'cancel') {
            return this.fail({message: 'OpenID authentication canceled'});
        }

        var self = this;
        var reqUrl = req.url;
        if (req.method === 'POST') {
            reqUrl = req.url + '?' + querystring.stringify(req.body);
        }

        this._relyingParty.verifyAssertion(reqUrl, function(err, result) {
            return self.verifyAssertionHandler(err, result, req);
        });
    } else {
        this.discoverOP(req);
    }
...
AdskStrategy.prototype.verifyAssertionHandler = function(err, result, req) {

    if (err) {
        return this.error(new InternalOpenIDError('Failed to verify assertion', err));
    }
    if (!result.authenticated) {

        if (req.query['openid.mode'] === 'setup_needed') {
            return this.success(undefined, 'Login Setup Required');
        }

        return this.error(new Error('OpenID authentication failed'));
    }

    var profile = this._parseProfileExt(result);
    var pape = this._parsePAPEExt(result);
    var oauth = this._parseOAuthExt(result);

    var arity = this._verify.length;
    if (this._passReqToCallback) {
        if (arity === 6) {
            this._verify(req, result.claimedIdentifier, profile, pape, oauth, this.verified.bind(this));
        } else if (arity === 5) {
            this._verify(req, result.claimedIdentifier, profile, pape, this.verified.bind(this));
        } else if (arity === 4 || this._profile) {
            // this._profile check covers the case where callback uses `arguments`
            // and arity == 0
            this._verify(req, result.claimedIdentifier, profile, this.verified.bind(this));
        } else {
            this._verify(req, result.claimedIdentifier, this.verified.bind(this));
        }
    } else {
        if (arity === 5) {
            this._verify(result.claimedIdentifier, profile, pape, oauth, this.verified.bind(this));
        } else if (arity === 4) {
            this._verify(result.claimedIdentifier, profile, pape, this.verified.bind(this));
        } else if (arity === 3 || this._profile) {
            // this._profile check covers the case where callback uses `arguments`
            // and arity == 0
            this._verify(result.claimedIdentifier, profile, this.verified.bind(this));
        } else {
            this._verify(result.claimedIdentifier, this.verified.bind(this));
        }
    }
};

AdskStrategy.prototype.verified = function(err, user, info) {
    if (err) {
...
AdskStrategy.prototype.discoverOP = function(req) {
 ...
 * @api public
 */
AdskStrategy.prototype.saveAssociation = function(fn) {
 ...
 * @api public
 */
AdskStrategy.prototype.loadAssociation = function(fn) {
 ...
 * @api public
 */
AdskStrategy.prototype.saveDiscoveredInfo =
    AdskStrategy.prototype.saveDiscoveredInformation = function(fn) {
 ...
 * @api public
 */
AdskStrategy.prototype.loadDiscoveredInfo =
    AdskStrategy.prototype.loadDiscoveredInformation = function(fn) {
 ...
 * @api private
 */
AdskStrategy.prototype._parseProfileExt = function(params) {
    var profile = {};
...
AdskStrategy.prototype._parsePAPEExt = function(params) {
    var pape = {};
...
AdskStrategy.prototype._parseOAuthExt = function(params) {
    var oauth = {};
